...
def process_pdf_directly_with_gemini(pdf_file, json_example: str, client: genai.Client, num_samples: int = 1) -> List[Dict]:
    ...
    try:
        model = "gemini-2.5-flash-preview-05-20" # Using an available and capable model for PDF processing
        ...
        response = client.models.generate_content(
            model=model,
            contents=contents,
            config=generate_content_config # Use generation_config
        )
...
def main():
    ...
    with tab2:
        ...
        with col1:
            st.subheader("📋 JSON Example")
            json_template = st.text_area("JSON Template:", height=300, key="json_template_tab2")
